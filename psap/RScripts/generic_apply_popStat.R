...
n.fam<-nrow(fam)
parents = c(unique(fam$V3[which(fam$V6==2 & fam$V3 !=0 )]),unique(fam$V4[which(fam$V6==2 & fam$V4 !=0)]))
...
for(i in fam$V2){
  ...
  if(length(which(a1 != 0 & a2 !=0 & a1 == a2)) > 0){
    exome.raw[which(a1 != 0 & a2 !=0 & a1 == a2),i] = "hom"
  }
...
exome = exome.raw[-unique(c(bl.remove,af.remove,lookup.remove)),]
...
male.x = data.frame(mom=c("ref","het","hom"),child=c("ref,hom","ref,hom","hom"),stringsAsFactors=F)
female.x = data.frame(parents=c("ref,ref","ref,het","ref,hom","hom,ref","hom,het","hom,hom"),child=c("ref,het","ref,het","het","het","het,hom","hom"),stringsAsFactors=F)

inconsistencies = function(row){
  parent.genos = paste(row[parents[1]],row[parents[2]],sep=",")
  count = 0
  for(i in children){
    if(row["Chr"] == "X" & fam$V5[which(fam$V2 == i)] == 1){
      genos = male.x[which(male.x$mom == row[parents[2]]),2]
      if(nchar(genos) > 3){
      ...
      if(row["Chr"] == "X" & fam$V5[which(fam$V2 == i)] == 2){
        genos = female.x[which(female.x$parents == parent.genos),2]
        if(nchar(genos) > 3){
          ok.genos = unlist(strsplit(genos,","))
        }else{
          ok.genos = genos
        ...
        genos = autosome[which(autosome$parents == parent.genos),2]
        if(nchar(genos) > 3){
          ok.genos = unlist(strsplit(genos,","))
        }else{
          ok.genos = genos
...
if(length(which(parents != 0)) == 2){
  print("1 true")
  tmp.exome = do.call(rbind,apply(exome,1,inconsistencies))
  tmp.exome = data.frame(tmp.exome,stringsAsFactors=F,check.names=F)
}else{
  print("2 true")
  tmp.exome = exome
}
